...
已完成源码阅读，并已根据用户反馈重新组织课程讲法，待理解检查。
...
本课第一版被用户指出不够符合学习偏好，已重新组织为：

```text
先建立心智模型
再给三条运行链路总览
再拆职责边界图
再讲 SessionTreeEntry 数据模型
再讲 buildSessionContext
再解释为什么不是 messages[]
再拆 Compaction / firstKeptEntryId / Cut Point / Split Turn
再拆 Branch Summary
最后用金融行业 Agent 和 Java / Dify 经验回填
```
...
3. 在金融行业 Agent 里，如果从一个方案切到另一个方案，Branch Summary 应该保留哪些信息？
